...
#define DECISION_WINDOW_SAMPLES 2
...
namespace geopm
{
    ...
    void CPUActivityAgent::validate_policy(std::vector<double> &in_policy) const
    {
        ...
        // We have no method of finding the actual uncore min and max at the moment
    ...
    void CPUActivityAgent::adjust_platform(const std::vector<double>& in_policy)
    {
        ...
        for (int domain_idx = 0; domain_idx < M_NUM_PACKAGE; ++domain_idx) {
            ...
            // Approach 4:
            // If the sample network time has been non-zero twice then increase
            //if (network_time_samples_avg == 1.0) {
            //    network_normalized = 1.0;
            //    ++m_network_normalized_frequency_requests;
            //}
            ...
            double package_core_req = f_core_efficient + f_core_range * scalability;

            //Clip core request within policy limits
            package_core_req = std::max(in_policy[M_POLICY_CPU_FREQ_EFFICIENT], package_core_req);
            package_core_req = std::min(in_policy[M_POLICY_CPU_FREQ_MAX], package_core_req);
            package_core_freq_request.push_back(package_core_req);
        }
        ...
        for (int domain_idx = 0; domain_idx < M_NUM_PACKAGE; ++domain_idx) {
            if(std::isnan(package_core_freq_request.at(domain_idx))) {
                package_core_freq_request.at(domain_idx) = in_policy[M_POLICY_CPU_FREQ_MAX];
            }
            ...
            if (package_core_freq_request.at(domain_idx) !=
                ...
                if (m_do_per_core == false) {
                    ...
                    m_package_freq_control.at(domain_idx).last_setting = package_core_freq_request.at(domain_idx);
    ...
    std::vector<std::pair<std::string, std::string> > CPUActivityAgent::report_host(void) const
    {
        ...
        result.push_back({"Xeon Core Frequency Requests", std::to_string(m_core_frequency_requests)});
